...
import type { MetadataApplyEnvelope, MetadataApplyPayload } from '@stego-labs/shared/contracts/cli';
...
export class MetadataCliClient {
  public async apply(
    ...
    return withJsonPayloadFile('stego-metadata-', payload, async (payloadPath) => {
      ...
      if (!invocation) {
        return { warning: this.reportWarning('Could not edit metadata because @stego-labs/cli is unavailable.', options) };
      }
      ...
      if (!parsed || parsed.ok !== true || parsed.operation !== 'apply' || !parsed.state) {
        return {
          warning: this.reportWarning('Could not edit metadata: @stego-labs/cli returned an unexpected response.', options)
        };
      }
